...
exports.saveLastMessage = functions.firestore.document('/chats/{chatId}/messages/{messageId}').onCreate((change , context ) => {
    ...
    return new  Promise((resolve , reject) => {
        ...
        chatRef.onSnapshot( snapChat => {
            ...
            let messsageRef = chatRef.collection('messages').doc(messageId).onSnapshot(snapMessage => {
                ...
                console.log('[USER TO ]' , userTo )
                 ...
                 db.collection("users").doc(decode64(userTo)).collection('contacts').doc(userFrom).set({
                     lestMessage : messageDoc.content ,
                     lestMessageTime : new Date()
                 }, {
                     merge : true
                 }).then(e=> {
